...
export class ExchangeRateService {
  ...
  getExchangeRateSunat(date: Date): Observable<ExchangeRateSunat> {
    ...
    return this.httpCliente
      .get<ExchangeRateAPISunat>(this.API_URL_SUNAT, { params })
      .pipe(
        // CAPTURE THE ERROR
        ...
        map((data) => {
          if (data.statusCode == 400) {
            throw new Error(
              'The exchange rate was not found for the selected date'
            );
          }
  ...
  getByLike(startat: Date, finalat: Date, origen: string, destin: string) {
    ...
    return this.httpCliente.get<DAOExchangeRate>(
      `${this.API_URL}${this.PATH_BILLING}/exchange-rate/by-like`,
      { params: params }
    );
  }

  postSave(data: ExchangeRateSunat) {
    return this.httpCliente.post<DAOExchangeRate>(
      `${this.API_URL}${this.PATH_BILLING}/exchange-rate`,
      data
    );
  }
  ...
  delDelete(registdate: Date, origen: string, destin: string) {
    let params = new HttpParams()
      ...
      .set('origen', origen)
      .set('destin', destin);
    return this.httpCliente.delete<DAOExchangeRate>(
      `${this.API_URL}${this.PATH_BILLING}/exchange-rate`,
      { params: params }
    );
  }
