...
export class DocumentInvoiceReceiptService {

  API_URL = environment.API_URL
  PATH_BILLING = environment.PATH_BILLING

  isStatusInvoiceReceipt = signal<'search' | 'register'>('search');
  isStatusInvoiceReceiptRegister = signal<boolean>(false);
  isStatusInvoiceReceiptRegisterDetail = signal<boolean>(false);
  isStatusInvoiceReceiptModify = signal<boolean>(false);
  isStatusInvoiceReceiptSave = signal<boolean>(false);

  setStatusInvoiceReceipt(data: 'search' | 'register') {
    this.isStatusInvoiceReceipt.set(data);
  }

  setStatusInvoiceReceiptRegister(data: boolean) {
    this.isStatusInvoiceReceiptRegister.set(data);
  }

  setStatusInvoiceReceiptRegisterDetail(data: boolean) {
    this.isStatusInvoiceReceiptRegisterDetail.set(data);
  }

  setStatusInvoiceReceiptModify(data: boolean) {
    this.isStatusInvoiceReceiptModify.set(data);
  }

  setStatusInvoiceReceiptSave(data: boolean) {
    this.isStatusInvoiceReceiptSave.set(data);
  }
