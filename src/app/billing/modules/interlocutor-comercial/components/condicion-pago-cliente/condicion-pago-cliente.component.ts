...
import { IntcomCondicionPagoView, InterlocutorComercialBasic } from '@billing-models/interlocutor-comercial.model';
...
export class CondicionPagoClienteComponent implements OnInit {
  ...
  displayedColumns: string[] = ['destyppaycon','limcre','updateby','updateat','operac']
  ...
  searchTypePaymentCondition(){
    ...
    this.businessPartnerService.getByCodintcomCondicionPago(this.codbuspar?.value)
    .subscribe({
      next:data =>{
  ...
  addTypePaymentCondition(){
    ...
    this.businessPartnerService.postSaveCondicionPago(intcomCondicionPago)
    .subscribe({
      next:data =>{
        if(data.status <= 0){
            this.dialog.open(DialogErrorAlertComponent,{
  ...
  deleteTypePaymentCondition(row: TypePaymentCondition){
    this.globalStatusService.setLoading(true)
    this.businessPartnerService.delDeleteCondicionPago(this.data.codbuspar,row.typpaycon)
    .subscribe({
      next:data =>{
        if(data.status <= 0){
            this.dialog.open(DialogErrorAlertComponent,{
