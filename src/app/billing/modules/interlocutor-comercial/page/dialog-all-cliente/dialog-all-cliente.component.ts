...
export class DialogAllClienteComponent {
  ...
  private buildForm(){
    this.formSearchCliente = this.formBuilder.group({
      typbuspar: [1,[Validators.required]],
      codbuspar: ['',[]],
      busnam: ['',[]],
      status: [false,[]]
    }, {
  ...
  searchInterlocutorComercial(){
    if(this.formSearchCliente.valid){
      ...
      console.log(this.formSearchCliente.value)
      this.businessPartnerService.getByPage(this.typbuspar?.value,this.codbuspar?.value,this.busnam?.value,this.status?.value,this.pageSize,this.pageIndex)
      .subscribe({
        next:data => {
  ...
  deleteInterlocutorComercial(row: InterlocutorComercial) {
    const dialogRef = this.dialog.open<boolean>(DialogDeleteQuestionComponent,{
      width: '400px',
      data: { status: 0, message: `Esta seguro en eliminar el Interlocutor Comercial ${row.codbuspar}, esta acción no es reversible!` }
    })
    dialogRef.closed.subscribe({
      next:data =>{
        if (data) {
          this.businessPartnerService.delDelete(row.codbuspar)
          .subscribe({
            next:data =>{
              if(data.status<=0){
                this.dialog.open(DialogErrorAlertComponent,{
  ...
  undeleteInterlocutorComercial(row: InterlocutorComercial) {
    const dialogRef = this.dialog.open<boolean>(DialogDeleteQuestionComponent,{
      width: '400px',
      data: { status: 1, message: `Esta seguro en activar el Interlocutor Comercial ${row.codbuspar}` }
    })
    dialogRef.closed.subscribe({
      next:data =>{
        if (data) {
          this.businessPartnerService.putUndelete(row.codbuspar)
          .subscribe({
            next:data =>{
              if(data.status<=0){
                this.dialog.open(DialogErrorAlertComponent,{
  ...
  byPageEvent(e: PageEvent){
    ...
    this.businessPartnerService.getByPage(this.typbuspar?.value,this.codbuspar?.value,this.busnam?.value,this.status?.value,this.pageSize,e.pageIndex)
    .subscribe(data =>{
      this.dataSource.getInit(data.page.content)
      this.globalStatusService.setLoading(false)
    })
