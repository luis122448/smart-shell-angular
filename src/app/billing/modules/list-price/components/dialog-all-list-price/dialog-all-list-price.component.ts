...
export class DialogAllListPriceComponent {
  ...
  deleteListPrice(row: ListPrice){
    const dialogRef = this.dialog.open<boolean>(DialogDeleteQuestionComponent,{
      width: '400px',
      data: { status: 0, message: `Are you sure to delete price list ${row.codlistprice} - ${row.descri} ? This action is not reversible` }
    })
    dialogRef.closed.subscribe({
      next:data =>{
        if (data) {
          this.listPriceService.delDelete(row.codlistprice)
          .subscribe({
            next:data =>{
              if (data.status<=0) {
                this.dialog.open(DialogErrorAlertComponent,{
