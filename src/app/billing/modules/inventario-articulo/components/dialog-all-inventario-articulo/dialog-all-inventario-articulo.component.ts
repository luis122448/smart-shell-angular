...
export class DialogAllInventarioArticleComponent implements OnInit {
  ...
  searchArticle(){
    if (this.formSearchInventarioArticle.valid) {
      ...
      this.articuloService.getPage(this.typinv?.value,this.codart?.value,this.descri?.value,this.pageSize,this.pageIndex)
      .subscribe({
        next:data =>{
  ...
  deleteArticle(row: Article){
    const dialogRef = this.dialog.open<boolean>(DialogDeleteQuestionComponent,{
      width: '400px',
      data: { status: 0, message: `Esta seguro de eliminar el Article ${row.codart}, esta acción no es reversible` }
    })
    dialogRef.closed.subscribe({
      next:data =>{
        if (data) {
          this.articuloService.delDelete(row.codart)
          .subscribe({
            next:data =>{
              if (data.status<=0) {
                this.dialog.open(DialogErrorAlertComponent,{
  ...
  undeleteArticle(row: Article){
    const dialogRef = this.dialog.open<boolean>(DialogDeleteQuestionComponent,{
      width: '400px',
      data: { status: 1, message: `Esta seguro de re-activar el Article ${row.codart} ?` }
    })
    dialogRef.closed.subscribe({
      next:data =>{
        if (data) {
          this.globalStatusService.setLoading(true);
          this.articuloService.putUndelete(row.codart)
          .subscribe({
            next:data =>{
              if (data.status<=0) {
                this.dialog.open(DialogErrorAlertComponent,{
  ...
  byPageEvent(e: PageEvent){
    ...
    this.articuloService.getPage(this.typinv?.value,this.codart?.value,this.descri?.value,this.pageSize,e.pageIndex)
    .subscribe(data =>{
      this.dataSource.getInit(data.page.content)
      this.globalStatusService.setLoading(false)
    })
